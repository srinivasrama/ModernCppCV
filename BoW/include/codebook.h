...
public:
  ...
  int size() const; // number of centroids/ code words
  std::vector<cv::Mat> descriptors() const { return descriptors_;};
  cv::Mat vocabulary() const{ return codebook_;};
  int total_features() const {return descriptors_.size();}; //number of input features
  bool empty() const{return !(descriptors_.size()>0);};

  //setter methods
  void set_max_iterations(int iter_){
    n_iter_=iter_;
  }
  void set_size(int n){
